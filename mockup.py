...
class MockupBoard(trfFirmata.Board):
...
class MockupPort(trfFirmata.Port):
    def __init__(self, board, port_number):
        ...
        for i in range(8):
            ...
            self.pins.append(MockupPin(self.board, pin_nr, type=trfFirmata.DIGITAL, port=self))
...
class MockupPin(trfFirmata.Pin):
    ...
    def write(self, value):
        if self.mode == trfFirmata.UNAVAILABLE:
            raise IOError("Cannot read from pin {0}".format(self.pin_number))
        if self.mode == trfFirmata.INPUT:
